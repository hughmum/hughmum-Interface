...
public class RabbitUtils implements RabbitTemplate.ConfirmCallback, RabbitTemplate.ReturnsCallback {
    ...
    /**
     * 向mq中投递发送短信消息
     * 这段代码是一个发送短信的方法，通过 RabbitMQ 实现异步发送短信，避免直接调用短信服务商的 API 导致的性能瓶颈和服务不可用问题
     * @param smsTo
     * @throws Exception
     */
    public void sendSms(SmsTo smsTo) {
        ...
        try {
            ...
            // 首先定义一个 messageId 变量，用于生成全局唯一的消息 ID。
            messageId = UUID.randomUUID().toString();
            //存储短信的重试次数、短信发送状态和短信内容等信息。
