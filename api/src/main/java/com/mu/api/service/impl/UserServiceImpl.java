...
import com.mu.api.common.*;
import com.mu.api.constant.UserConstant;
import com.mu.api.feign.ApiOrderFeignClient;
import com.mu.api.mapper.InterfaceInfoMapper;
import common.Exception.BusinessException;
import com.mu.api.mapper.AuthMapper;
import com.mu.api.mapper.UserMapper;
import com.mu.api.model.dto.user.UserBindPhoneRequest;
import com.mu.api.model.dto.user.UserLoginBySmsRequest;
import com.mu.api.model.dto.user.UserQueryRequest;
import com.mu.api.model.dto.user.UserRegisterRequest;
import com.mu.api.model.entity.Auth;
import com.mu.api.model.entity.User;
import com.mu.api.model.vo.UserVO;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    public long userRegister(UserRegisterRequest userRegisterRequest ,HttpServletRequest request) {
        ...
        //对userAccount字符串进行同步操作，使用userAccount.intern()作为锁对象。
    ...
    public User getLoginUser(HttpServletRequest request) {
        ...
        /*
        * 通过Spring Security的SecurityContextHolder获取当前的Authentication对象，并将其强制转换为User对象，保存在currentUser变量中。
        需要注意的是，Authentication对象是Spring Security框架中的一个重要概念，它代表当前用户的身份信息。
        * 而SecurityContextHolder则是一个与线程绑定的上下文对象，用于保存当前用户的安全上下文信息
        * */
    ...
    public BaseResponse updateUserPass(String password, HttpServletRequest request) {
        ...
        String value = null;
        try {
            cookie = cookies[1]; //cookie[1] 装的value是token的值
            value= cookie.getValue(); //获得了jwt
        }catch (Exception e){
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        //从redis中拿到加密后的信息
        String s = (String) redisTemplate.opsForValue().get(value);
        String[] strings = mobileSignature.decodeHex(s); //解密
        String username = strings[0];
        if (null == username){
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
    ...
    public BaseResponse checkUserLogin(HttpServletRequest request, HttpServletResponse response) {
        // 先判断是否已登录
        // request.getUserPrincipal()方法是从HTTP请求中获取当前用户的Principal对象，如果该对象不为null，则说明用户已经通过认证并且已经登录。Principal对象通常包含用户的身份信息，例如用户名、ID等。在这段代码中，如果userPrincipal不为null，则说明用户已经登录，可以根据该对象中的信息进行后续的操作。
        //需要注意的是，getUserPrincipal()方法只能获取通过基于表单或基于HTTP基本认证的方式登录的用户信息。如果用户是通过其他方式登录的，例如OAuth2或OpenID Connect等，则需要使用相应的认证库来获取当前用户的身份信息。
        Principal userPrincipal = request.getUserPrincipal();
        if (userPrincipal!=null){
            String name = userPrincipal.getName();
            Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
            ...
            /*虽然通过request.getUserPrincipal()方法可以判断用户是否已经登录，但是该方法只能判断用户是否通过基于表单或基于HTTP基本认证的方式登录，对于其他方式的登录，例如OAuth2或OpenID Connect等，该方法无法判断用户是否登录。因此，为了支持多种登录方式，该方法还需要从Cookie中获取相应的信息，以判断用户是否已经登录。
具体来说，如果用户是通过“记住我”功能登录的，那么该方法就需要从Cookie中获取相应的信息，以判断用户是否已经登录。因为在“记住我”功能登录时，用户的登录状态会通过Cookie进行持久化，即使用户关闭了浏览器，也可以在下次打开浏览器时直接登录。因此，在这种情况下，request.getUserPrincipal()方法无法判断用户是否已经登录，需要通过解析Cookie中的信息来判断用户是否已经登录。*/
            Cookie[] cookies = request.getCookies();
    ...
    public BaseResponse bindPhone(UserBindPhoneRequest userBindPhoneRequest,HttpServletRequest request) {
        String captcha = userBindPhoneRequest.getCaptcha(); // 图形验证码
        ...
        // 加个锁
        synchronized (userAccount.intern()){ //将字符串加入字符串池中，并返回字符串池中的字符串。这样，同一个字符串在不同的地方使用时，都可以共用一个锁对象。
            //手机号不能重复
            String getMapperPhone = userMapper.selectPhone(mobile);
            if (null != getMapperPhone){
                throw new BusinessException(ErrorCode.PARAMS_ERROR,"手机号已经被注册");
    ...
    public BaseResponse getGithubStars() {
        ...
        try {
            listContent=HttpUtil.get("https://img.shields.io/github/stars/hughmum?style=social");
        }catch (Exception e){
    ...
    private LoginUserVo initUserLogin(UserDetails user,HttpServletResponse response){
        ...
        response.addCookie(cookie);
        CookieUtils cookieUtils = new CookieUtils();
        //autoLoginContent表示自动登录所需要的信息，例如用户名、密码等。当用户勾选了“自动登录”选项后，服务器会将该信息保存到cookie中，以便下次用户打开网站时，可以自动登录，无需重新输入用户名和密码。
        String autoLoginContent = cookieUtils.generateAutoLoginContent(loginUserVo.getId().toString(), loginUserVo.getUserAccount());
        //创建了一个名为CookieConstant.autoLoginAuthCheck，值为autoLoginContent的Cookie对象
        Cookie cookie1 = new Cookie(CookieConstant.autoLoginAuthCheck, autoLoginContent);
        //设置cookie的路径为"/"，表示该cookie对网站的所有路径都有效。
        cookie1.setPath("/");
        cookie.setMaxAge(CookieConstant.expireTime);
